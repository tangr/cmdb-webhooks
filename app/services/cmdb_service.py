...
from app.models.cmdb_reqlog import CmdbReqLog, CmdbReqLogCreate
...
def log_cmdb_request(session: SessionDep, log_entry: CmdbReqLogCreate):
    """Log CMDB request to database based on configuration"""

    # Database logging
    if settings.enable_database_logging:
        try:
            db_log = CmdbReqLog(**log_entry.model_dump())
            session.add(db_log)
            session.commit()
        except Exception as e:
            # If database logging fails and console logging is enabled, log the error
            if settings.enable_console_logging:
                logger.error(f"Failed to save CMDB log to database: {str(e)}")
...
async def process_cmdb_request(
    ...
    """Process CMDB proxy request with API key verification and forwarding"""

    # Verify IP whitelist (empty list = deny all)
    verify_webhook_ip_whitelist(request, settings.webhook_ip_whitelist)

    # Verify CMDB webhook API key (skip if no API keys configured)
    verify_cmdb_webhook(request, api_key)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            log_cmdb_request(session, log_entry)
